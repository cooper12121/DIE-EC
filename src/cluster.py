...
def print_results(all_mentions, scorer_out_file):
    ...
    #gold_lable_dict
    gold_lable_dict=defaultdict(list)
    ...
    pred_cluster_list=[]
    for cluster_id, cluster in all_clusters.items():
        if  len(cluster) == 1  and len(gold_lable_dict[cluster[0].coref_chain])==1:
...
if __name__ == '__main__':
    ...
    argv=['--tmf','./datasets/CDEC-zh/final_processed/test_event_validated.json',
          '--predictions','./model/rhetorical/pair_pred_no_选择关系.pickle','--alt',0.925]
    ...
    log_param =  dt_string
    # create_logger_with_fh("checkpoints/cluster_"+dt_string+"_rhetorical")
    create_logger_with_fh("checkpoints/cluster_2024-01-27_10-44_rhetorical")
    logger.info(f"\nRuning rst ablation expreiments:{_predictions_file}")
