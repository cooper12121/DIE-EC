...
class EmbedTransformersGenerics(object):
	def __init__(self, max_surrounding_contx,
				 finetune=False, use_cuda=True,use_arguments=False):
	...
	def get_mention_full_rep(self, mention):
		...
		if not self.finetune:
			...
			last_hidden_span = self.model(sent_ids).last_hidden_state #[batch,sequence,hidden_state],
	...
	def extract_mention_surrounding_context(mention):
		...
		# assert ret_mention == mention.tokens_str.split(" ")
			assert "".join(i for i in ret_mention)==mention.tokens_str
			assert ret_context_before + ret_mention + ret_context_after == mention.mention_context
		except Exception as e:
			print(e,ret_mention,mention.tokens_str)
			pass
	...
	def mention_feat_to_vec(self, mention):
		...
		all_sent_toks = [[101] + cntx_before + ment_span + cntx_after + [102]]
	...
	def get_arguments(self,mention,sent_tokens):
		...
		arg0_start,arg0_end,arg0_span=arguments[0]
		arg0_start_idx,arg0_end_idx=self.wordpairID_2_tokenpairID(arg0_span,arg0_start,arg0_end,sent_tokens)
	...
	def wordpairID_2_tokenpairID(self,span, wordindex_left, wordindex_right, sent_tokens):
		...
		span_id_list = self.tokenizer.convert_tokens_to_ids(span_token_list)
