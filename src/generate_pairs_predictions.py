...
def generate_pred_matrix(inference_model, topic):
    ...
    with torch.no_grad():
        for chunk in tqdm(pairs_chunks,desc="pairs_chunks"):
...
def get_pairwise_model():
    ...
    pairwize_model = torch.load(_model_file,map_location=device)
...
if __name__ == '__main__':
    ...
    argv=['--tmf','../datasets/CDEC-zh/final_processed/test_event_validated.json',
          '--tef','../datasets/CDEC-zh/final_processed/embed/test_event_validated_roberta_large.pickle',
          '--ten','../datasets/CDEC-zh/final_processed/gat_embed/test_event_validated_roberta_large.pickle',
          '--mf','../checkpoints/zh_pairwise_modeldev_ratio_-1_iter_10.pickle',
          '--out','../model/rhetorical/pair_pred_no_让步关系.pickle',# ablation experiments
          ]
    ...
    logger.info("Running agglomerative clustering with model:" + type(_model).__name__+_outfile)
    predict_and_save()
