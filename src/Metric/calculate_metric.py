...
def calculate_cluster(data,pred_labels,gold_labels):
    ...
    """
    cluster_pred=defaultdict(set)
    cluster_pred_id=0
    pred_index2cluster_dict = defaultdict(int)
    ...
    for index,mention_pair in enumerate(data):
        ...
        if pred_labels[index]==1:
            if mention1_pred_cluster_id!=None:
                if mention2_pred_cluster_id==None:
                    cluster_pred[mention1_pred_cluster_id].add(mention2_index)
                else:
                ...
                pred_index2cluster_dict[mention2_index]=mention1_pred_cluster_id
            else:
                if mention2_pred_cluster_id==None:
            ...
            if mention1_pred_cluster_id!=None:
                cluster_pred[mention1_pred_cluster_id].add(mention1_index)
            else:
                cluster_pred[cluster_pred_id].add(mention1_index)
                pred_index2cluster_dict[mention1_index]=cluster_pred_id
                cluster_pred_id+=1
            if mention2_pred_cluster_id!=None:
                cluster_pred[mention2_pred_cluster_id].add(mention2_index)
            else:
                cluster_pred[cluster_pred_id].add(mention2_index)
                pred_index2cluster_dict[mention2_index]=cluster_pred_id
                cluster_pred_id+=1
    ...
    #gold
        if(gold_labels[index]==1):
            if mention1_gold_cluster_id!=None:
                if mention2_gold_cluster_id==None:
                    cluster_gold[mention1_gold_cluster_id].add(mention2_index)
                else:
                ...
                gold_index2cluster_dict[mention2_index]=mention1_gold_cluster_id
            else:
                if mention2_gold_cluster_id==None:
            ...
            if mention1_gold_cluster_id!=None:
                cluster_gold[mention1_gold_cluster_id].add(mention1_index)
            else:
                cluster_gold[cluster_gold_id].add(mention1_index)
                gold_index2cluster_dict[mention1_index]=cluster_gold_id
                cluster_gold_id+=1
            if mention2_gold_cluster_id!=None:
                cluster_gold[mention2_gold_cluster_id].add(mention2_index)
            else:
                cluster_gold[cluster_gold_id].add(mention2_index)
                gold_index2cluster_dict[mention2_index]=cluster_gold_id
                cluster_gold_id+=1
    return cluster_pred,cluster_gold
def process_cluster(predict_label,gold_label,data,split):
    ...
    cluster_pred=[list(i)for i in cluster_pred.values() if len(i)!=0]
    cluster_gold=[list(i)for i in cluster_gold.values() if len(i)!=0]
    print(f"len_clusterr_pred:{len(cluster_pred)},len_cluster_gold:{len(cluster_gold)}")
    metrics_result(cluster_pred,cluster_gold,split)
def metrics_result(cluster_pred,cluster_gold,split):
    with open(f"./{split}_metric.txt","w")as f:
...
def read_prediction_gold_label(predict_labels=None,gold_labels=None):
    ...
    if predict_labels!=None:
        ...
        with open('../../model/wec_best_predict_dev_label.json','r',encoding='utf-8')as f:
            data=json.load(f)
            f.close()
...
def read_validation_dataset(data=None):
    ...
    if data!=None:
        ...
        data = pickle.load(open('../../model/wec_best_dev_validation_dataset.pickle','rb'))
        # print(data)
        return data
