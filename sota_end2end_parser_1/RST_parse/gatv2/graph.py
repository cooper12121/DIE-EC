...
class Graph:
    def __init__(self, graph_info_list,edu_list,node_features=1024, edge_features=1024,):
    ...
    def build_graph(self):
        # with open('../RST_example/graph_info_list.json','r',encoding='utf-8')as f:
        #     graph_info_list = json.load(f)
        #     f.close()
        # print(f"size:{len(graph_info_list)}")
        src =[]
        dst = []
    ...
    def add_node_features(self):
        ...
        h = torch.rand((5, self.node_features))
        for i in range(h.shape[0]):
            embed.weight.data[edu2node[i]] = h[i]
