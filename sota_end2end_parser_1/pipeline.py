...
path_to_jar = 'model/stanford-corenlp-full-2018-02-27'
...
model = torch.load("data/models_saved/model.pth",map_location=torch.device('cpu'))
segmenter = torch.load(os.path.join(MODEL_SAVE_SEG, "EN_200.model"), map_location=torch.device('cpu'))
...
def do_seg(sents_dt_, rt_path=None):
    ...
    for doc_dt in result_dt:
        ...
        edus_all.append("")
    # write to file
    # write_iterate(edus_all, rt_path, append_=False)
    return edus_all
...
def gen_batch_iter(random_instances, batch_s=BATCH_SIZE):
    ...
    while offset < num_instances:
        ...
        num_batch = len(batch)
        lengths = np.zeros(num_batch, dtype=int)
...
def data_ids_prep(num_batch, max_seq_len, batch):
    ...
    """
    words_all = []
    word_inputs = np.zeros([num_batch, max_seq_len], dtype=np.longlong)
    word_elmo_embeddings = np.zeros([num_batch, max_seq_len, 1024], dtype=float)
    pos_inputs = np.zeros([num_batch, max_seq_len], dtype=np.longlong)
...
def prepare_dt(seg_edus):
    ...
    tmp_tree = []
    for line in lines:
        if len(line.strip()) == 0 and len(tmp_tree) > 0: #这里好像是分隔多个树
...
if __name__ == "__main__":
    ...
    with open("data/e2e/raw.txt", "r") as f:
        sentences = f.readlines()
    sents_dt = prep_seg(sentences)
