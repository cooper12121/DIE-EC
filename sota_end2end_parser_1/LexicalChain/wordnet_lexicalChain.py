...
def get_edu_chain(edu_list):
    ...
    for edu in edu_list:
        ...
        # edu_noun_relation.append()
    lexical_chains = cal_similarity(nouns, relation)
    return lexical_chains

def cal_similarity(edu_noun_relation_list):
    lexical_chains=[]
    for i in range(len(edu_noun_relation_list)):
        if len(edu_noun_relation_list[i][0])==1 and edu_noun_relation_list[i][0][0]=='none':continue
        for j in range(i+1,len(edu_noun_relation_list)):
            if len(edu_noun_relation_list[j][0]) == 1 and edu_noun_relation_list[j][0][0] == 'none': continue
            has_enough_chain=get_lexical(edu_noun_relation_list[i][0],edu_noun_relation_list[i][1],edu_noun_relation_list[j][0],edu_noun_relation_list[j][1])
            if has_enough_chain:
...
def get_lexical(nouns_1,relations_1,nouns_2,relations_2):
    lexical_chain_num=0
    #假定当链数超过一定数量时才认为两个edu之间存在词汇链，两个edu之间加一条边。
...
def readfile():
    infile_list = ["../RST_parse/data_process/ecb+/rst/dev_Event_gold_mentions_processed.json",
                   "../RST_parse/data_process/ecb+/rst/test_Event_gold_mentions_processed.json",
                   "../RST_parse/data_process/ecb+/rst/train_Event_gold_mentions_processed.json"]
...
def r_test():
    edu_list = ['an air canada plane made an abrupt landing', 'and left the runway at the halifax airport in bad weather ,', 'but there were no immediate reports of injuries ,', 'authorities said early sunday .', '']
    chain = get_edu_chain(edu_list)
    print(chain)
...
if __name__ == "__main__":
    ...
    threshold = 0.5
    nouns, relation = get_chain(input_txt)
    print(nouns,relation)
    lexical_chains = cal_similarity(nouns, relation)
    print(lexical_chains)
    r_test()
    readfile()
# 若用此方法做要做的修改
