...
from treebuilder.partptr.Rst_tree import rst_tree
...
class PartitionPtrParser:
    ...
    def init_session(self, edus):
        ...
        edu_encoded, e_masks = self.model.encode_edus((e_input_words, e_input_poses, e_input_masks))
    ...
    def build_tree(self, edus, splits, nuclears, relations):
        ...
        # print(f"type:{type(edus[split])}")
        if split - left == 1:#leaf node
            left_node = rst_tree(temp_edu=edus[left].text,type_=nuclear,rel=ftype)
            # left_node = edus[left]
        else:
            left_node = self.build_tree(edus, splits, nuclears, relations)

        if right - split == 1:
            right_node = rst_tree(temp_edu=edus[split].text, type_=nuclear, rel=ftype)
        ...
        root = rst_tree(l_ch=left_node, r_ch=right_node,child_rel=ftype, type_="root", rel=ftype)
        return root
